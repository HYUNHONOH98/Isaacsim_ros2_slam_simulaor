...
# Policy parameters
parser.add_argument("--policy-path", type=str, default="/workspace/isaaclab/source/ros2/isaac-ros/assets/0820_f9/0820_f9_policy.pt", help="Path to the policy file")
parser.add_argument("--nav-policy-path", type=str, default="/workspace/isaaclab/source/ros2/isaac-ros/assets/0820_f9_nav/policy.pt", help="Path to the policy file")
...
start_time = free_iter

SLOW_BOUND = 0.4
MAX_LIN_VEL = 0.3
MAX_ANG_VEL = 0.3
NAV_HZ = 5
ERROR_THRESHOLD = 0.05  # m
NUM_AVG = 70
pos_command_log = np.zeros((0, 4))
STOPED=False
# =============================== Main iteration ===============================
...
while simulation_app.is_running():
    ...
    if ( current_iter % 4 == 0 or is_first_released ) and est_midsole_pos is not None and est_midsole_quat is not None:
        ...
        forward_w = quat_apply(est_midsole_quat.astype(np.float32), np.array([1., 0., 0.]).astype(np.float32))
        ...
        target_vec = target_pos_w - est_midsole_pos
        target_vec[2] = 0.0
        pos_command_b = quat_rotate_inverse(yaw_quat(est_midsole_quat).astype(np.float32), target_vec.astype(np.float32))
        midsole_pos_w, midsole_quat_w = read_midsole_pose()
        # print("midsole diff : ", np.linalg.norm(midsole_pos_w[:2] - est_midsole_pos[:2]))
        ...
        if True or USE_AVG_POS_CMD :
            pos_command_log = np.vstack([pos_command_log, pos_command[None, :]])
            POS_WINDOW = 20
            n = min(len(pos_command_log), POS_WINDOW)
            if n > 0:
                pos_command_smoothed = np.mean(pos_command_log[-n:], axis=0)  # shape (4,)
            else:
                pos_command_smoothed = pos_command
        ...
        slam_errors = np.vstack((slam_errors, np.linalg.norm(midsole_pos_w[:2] - est_midsole_pos[:2]).reshape((1,1))))
        pos_command_bs = np.vstack((pos_command_bs, np.linalg.norm(pos_command_b[:2]).reshape((1,1))))
        
        gt_errors = np.vstack((gt_errors, np.linalg.norm(target_pos_w[:2] - midsole_pos_w[:2]).reshape((1,1))))
        ...
        if pos_command_bs.shape[0] > NUM_AVG:
            if np.mean(pos_command_bs[-NUM_AVG:, :].reshape((NUM_AVG,))) < 0.1 \
               and STOPED and errors_per_pose[current_target_pose_idx][1] == [] :
                ...
                pos_error = round(np.linalg.norm(midsole_pos_w[:2] - target_pos_w[:2]), 4)
                ori_error = round(np.abs(heading_error[0]), 4)
                stop_time = (current_iter- start_time)*dt
                print(f"pos error : {pos_error:.4f}")
                print(f"ori error : {ori_error:.4f}")
        ...
        if current_iter % 40 == 0:
            ...
            # print(f"pos command : {pos_command}")
            # print(f"velocity command : {vel_command_b}")

        if fixed is not True and (STOPED or np.linalg.norm(vel_command_b) < 0.05):
            STOPED = True
            phase = 0.
            sin_p, cos_p = np.sin(2*np.pi*phase), np.cos(2*np.pi*phase)
            vel_command_b[:] = (0, 0, 0)
